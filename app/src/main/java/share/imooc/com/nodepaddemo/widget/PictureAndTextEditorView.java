...
import share.imooc.com.nodepaddemo.utils.MediaPlayerManager;
...
public class PictureAndTextEditorView extends EditText{
    ...
    private Bitmap voiceBitmap=BitmapFactory.decodeResource(getResources(), R.mipmap.speak);
        ...
        if (mContentList.size() > 0) {
            for (String str : mContentList) {
                if (str.indexOf(mBitmapTag) != -1) {//判断是否是图片地址
                    ...
                    //插入声音图片
                    String path = str.replace(mVoiceBitmapTag, "");//还原地址字符串
                    insertVoiceBitmap(path);
                }else {
                    //插入文字
...
//插入声音图标
    private SpannableString insertVoice(final String path, Bitmap bitmap) {
        Editable edit_text = getEditableText();
        int index = getSelectionStart(); // 获取光标所在位置
        //插入换行符，使图片单独占一行
        SpannableString newLine = new SpannableString("\n");
        ...
        edit_text.insert(index, newLine);//插入图片前换行
        // 创建一个SpannableString对象，以便插入用ImageSpan对象封装的图像
        final String newpath = mVoiceBitmapTag + path + mVoiceBitmapTag;
        ...
        // 用ImageSpan对象替换你指定的字符串
        spannableString.setSpan(imageSpan, 0, newpath.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        spannableString.setSpan(clickableSpan, 0, newpath.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        // 将选择的图片追加到EditText中光标所在位置
        if (index < 0 || index >= edit_text.length()) {
            edit_text.append(spannableString);
        } else {
    ...
    public void insertBitmap(String path) {
        Bitmap bitmap = getSmallBitmap(path, 480, 800);
        SpannableString spa=insertBitmap(path, bitmap);
    }

    public void  insertVoiceBitmap(String path) {
        SpannableString spa=insertVoice(path,voiceBitmap);
    }
